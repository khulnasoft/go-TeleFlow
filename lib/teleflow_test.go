...
func TestError_Retry_With_Custom_Config(t *testing.T) {
	...
	c := lib.NewAPIClient(teleflowApiKey, &lib.Config{BackendURL: lib.MustParseURL(subscriberService.URL), RetryConfig: &lib.RetryConfigType{RetryMax: 5, InitialDelay: 0 * time.Second}})
...
func TestError_Retry_With_Default_Config(t *testing.T) {
	...
	c := lib.NewAPIClient(teleflowApiKey, &lib.Config{BackendURL: lib.MustParseURL(subscriberService.URL)})

...
import (
	...
	"github.com/khulnasoft/go-teleflow/lib"
...
const (
	teleflowApiKey  = "test-API-key"
	teleflowEventId = "test-teleflow"
)
...
func TestEventServiceTrigger_Success(t *testing.T) {
	...
	eventService := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		t.Run("Header must contain ApiKey", func(t *testing.T) {
			...
			assert.True(t, strings.Contains(authKey, teleflowApiKey))
		...
		t.Run("Request is as expected", func(t *testing.T) {
			fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger.json"), &expectedTokenRequest)
			assert.Equal(t, expectedTokenRequest, receivedBody)
		})

		var resp lib.EventResponse
		fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger_response.json"), &resp)
	...
	fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger.json"), &triggerPayload)

	c := lib.NewAPIClient(teleflowApiKey, &lib.Config{BackendURL: lib.MustParseURL(eventService.URL)})
	_, err := c.EventApi.Trigger(ctx, teleflowEventId, triggerPayload)
...
func TestEventServiceTriggerForTopic_Success(t *testing.T) {
	...
	eventService := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		t.Run("Request is as expected", func(t *testing.T) {
			fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger_topic_recipient.json"), &expectedTokenRequest)
			assert.Equal(t, expectedTokenRequest, receivedBody)
		})
	...
	ctx := context.Background()
	fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger_topic_recipient.json"), &triggerPayload)
...
func TestBulkTriggerEvent_Success(t *testing.T) {
	...
	eventService := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		t.Run("Request is as expected", func(t *testing.T) {
			fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger_bulk.json"), &expectedTokenRequest)
			assert.Equal(t, expectedTokenRequest, receivedBody)
		})

		var resp []lib.EventResponse
		fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger_bulk_response.json"), &resp)
	...
	ctx := context.Background()
	fileToStruct(filepath.Join("../testdata", "teleflow_send_trigger_bulk.json"), &triggerPayload)
...
func TestBroadcastEventToAll_Success(t *testing.T) {
	...
	eventService := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		t.Run("Request is as expected", func(t *testing.T) {
			fileToStruct(filepath.Join("../testdata", "teleflow_broadcast_event_to_all.json"), &expectedTokenRequest)
			assert.Equal(t, expectedTokenRequest, receivedBody)
		})
	...
	ctx := context.Background()
	fileToStruct(filepath.Join("../testdata", "teleflow_broadcast_event_to_all.json"), &triggerPayload)
